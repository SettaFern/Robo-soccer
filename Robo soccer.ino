...
#define rot_Ki 0.0
#define rot_Kd 0.0
#define sp_rot 156       // ค่า setpoint ที่ลูกบอลอยู่ตรงกลางกล้องแกน x  320/2 = 160
...
float spFli = 130;         // ค่า setpoint ที่ยอมให้ลูกบอลอยู่ใกล้หุ่นมากที่สุด อาจเริ่มที่จุดกลางจอ แกน Y
      ...
      if (abs(pvYaw) < alignErrorGap) {  //เมื่อทิศอยู่ในค่าที่รับได้
        ...
        if (abs(rot_error) < rotErrorGap) {  //ถ้าลูกบอลอยู่ตรงกลางให้ทำการยิง
          ...
          while (1) {  //เดินหน้าตรงความเร็ว 100%
            getIMU();
            heading(60,90,0); delay(500);
            if ((pixy.updateBlocks() && pixy.sigSize[2]) && (pixy.updateBlocks() && pixy.sigSize[1])) {
              //pixy.sigInfo[2][0].x;
              //pixy.sigInfo[2][0].y;
              //beep();
              heading(60,90,0);
